...
describe('retryFetch', () => {
  ...
  it('should retry the number of times given', async () => {
    fetchMock.mockResponse('error', { status: 500 })
    await expect(retryFetch(Url, undefined, { retries: 10, delay: async () => 0 })).rejects.toThrow(RequestError)
  ...
  it('should allow custom delay function', async () => {
    ...
    const delayFn = vi.fn<DelayFn>(async (attempt, _, base) => attempt === 0 ? 0 : base())
