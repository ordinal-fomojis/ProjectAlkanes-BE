...
export default class Random {
  ...
  randomChooseMany<T>(array: T[], amount: number) {
    if (amount > array.length) {
      throw new Error(`Cannot choose ${amount} items from an array with length ${array.length}`)
    }
    const chosen: T[] = []
  ...
  randomString({ length, charset } : RandomStringOptions = {}) {
    ...
    charset ??= 'alphanum'
    return [...Array(length)].map(() => this.randomChoice(CHARSETS[charset!])).join('').trim()
  }
  ...
  randomHex(length: number) {
    return [...Array(length)].map(() => this.randomIntLessThan(16).toString(16)).join('')
  }
  ...
  randomInscriptionId() {
    ...
    return `${this.randomTransactionId()}i${index}`
  }
